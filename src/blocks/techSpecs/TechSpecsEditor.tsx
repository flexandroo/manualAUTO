...
interface Props {
  data: TechSpecsData;
  onChange: (data: TechSpecsData) => void;
}

export function TechSpecsEditor({ data, onChange }: Props) {
  const updateProperty = (i: number, field: 'key' | 'value', value: string) => {
    const newProps = [...data.properties];
    newProps[i] = { ...newProps[i], [field]: value };
    onChange({ ...data, properties: newProps });
  };
  const addProperty = () =>
  ...
  const updateCell = (rowIdx: number, colIdx: number, value: string) => {
    const newRows = data.table.rows.map((r, i) =>
      i === rowIdx ? r.map((c, j) => (j === colIdx ? value : c)) : r
    );
    onChange({ ...data, table: { ...data.table, rows: newRows } });
  };
  const updateHeader = (i: number, value: string) => {
    const newHeaders = [...data.table.headers];
    newHeaders[i] = value;
    onChange({ ...data, table: { ...data.table, headers: newHeaders } });
  };
  const addRow = () => {
    ...
    <div>
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          ...
          <IconBtn onClick={addProperty} variant="primary">
            <Plus size={14} />
          </IconBtn>
        </div>
        <div className="space-y-2">
          {data.properties.map((p, i) => (
            <div key={i} className="flex gap-2 items-start">
              ...
              />
              <IconBtn onClick={() => removeProperty(i)} variant="danger">
                <Trash2 size={14} />
              </IconBtn>
            </div>
          ))}
