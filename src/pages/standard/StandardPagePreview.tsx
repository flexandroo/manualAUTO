...
import { balancedSplitPoint } from '../../utils/layoutEstimator';
...
export function StandardPagePreview({ data }: Props) {
  if (data.twoColumn && data.elements.length > 0) {
    const split = balancedSplitPoint(data.elements);
    const left = data.elements.slice(0, split);
    const right = data.elements.slice(split);
    return (
      <PdfPageShell
