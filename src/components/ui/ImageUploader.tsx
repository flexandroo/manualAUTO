...
export function ImageUploader({ value, onChange, label, hint, aspectRatio = '4/3' }: Props) {
  ...
  const accept = (file: File | null | undefined) => {
    ...
    fileToDataUrl(file).then(onChange).catch(() => setError('Помилка зчитування файлу'));
  };
    ...
    <div className="mb-5">
      ...
      >
        {value ? (
          ...
          <button
            onClick={() => inputRef.current?.click()}
            className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-slate-500 hover:text-slate-300"
          >
            <ImagePlus size={32} />
